...
from colossus.mailing.forms import SubscribeForm
...
def subscribe(request, mailing_list_uuid):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('api:confirm_subscription', mailing_list_uuid=mailing_list_uuid)
    else:
        form = SubscribeForm(mailing_list=mailing_list)
    return render(request, 'mailing/subscription/subscription_form.html', {
...
def confirm_subscription(request, mailing_list_uuid):
    mailing_list = MailingList.objects.get(uuid=mailing_list_uuid)
    return render(request, 'mailing/subscription/confirm_subscription.html', {'mailing_list': mailing_list})
